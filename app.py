...
class Alumni(db.Model):
    """
    Alumni table definition with robust constraints and defaults.
    """
    ...
    # Basic personal info
    full_name = db.Column(db.String(255), nullable=False)         # Required
    email = db.Column(db.String(255), unique=True, nullable=False)  # Required & must be unique
    contact_number = db.Column(db.String(15), nullable=False)     # Required: digits only, 7–15 chars
    dob = db.Column(db.Date, nullable=False)                      # Required: must be in past
    gender = db.Column(db.String(10), nullable=False)             # Required: Male/Female/Other

    # Academic details
    course = db.Column(db.String(100), nullable=False)            # Required
    department = db.Column(db.String(100), nullable=False)        # Required
    admission_year = db.Column(db.Integer, nullable=False)        # Required: reasonable years only
    batch_year = db.Column(db.Integer, nullable=False)            # Required: >= admission year

    enrollment_number = db.Column(db.String(100), unique=True, nullable=False)  # Required & unique
    ...
    # Optional salary/package
    student_package = db.Column(db.Numeric(15, 2))                # Optional: decimal up to 15 digits, 2 decimals

    # Address & location
    address = db.Column(db.Text, nullable=False)                  # Required
    city = db.Column(db.String(100), nullable=False)              # Required
    state = db.Column(db.String(100), nullable=False)             # Required
    country = db.Column(db.String(100), nullable=False)           # Required

    # Additional info
    achievements = db.Column(db.Text)

    # Auto timestamp: record when alumni entry was created
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    # ------------------------------------------
    # ✅ Extra Check Constraints for Data Quality
    # ------------------------------------------
    __table_args__ = (
        # Ensure email contains '@'
        db.CheckConstraint("position('@' IN email) > 1", name="check_email_format"),

        # Ensure contact_number is digits only & proper length
        db.CheckConstraint(
            "char_length(contact_number) BETWEEN 7 AND 15 AND contact_number ~ '^[0-9]+$'",
            name="check_contact_number"
        ),

        # Ensure DOB must be in the past
        db.CheckConstraint("dob < CURRENT_DATE", name="check_dob_past"),

        # Ensure gender must be Male, Female, or Other
        db.CheckConstraint("gender IN ('Male', 'Female', 'Other')", name="check_gender"),

        # Ensure admission year is realistic
        db.CheckConstraint(
            "admission_year BETWEEN 1900 AND EXTRACT(YEAR FROM CURRENT_DATE)",
            name="check_admission_year"
        ),

        # Ensure batch year is >= admission year
        db.CheckConstraint("batch_year >= admission_year", name="check_batch_year"),
    )

    def __repr__(self):
        return f"<Alumni {self.id} - {self.full_name}>"
